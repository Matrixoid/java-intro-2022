...
public class WsppCountLastL {
    public static void main(String[] args) {
        ...
        Map<String, IntListHard> result = new LinkedHashMap<>();
        try {
                    ...
                    if (Character.isLetter(sb.charAt(i)) || sb.charAt(i) == '\'' || Character.getType(sb.charAt(i)) == Character.DASH_PUNCTUATION) {
                        ...
                        if (!word.isEmpty()) {
                            ...
                            if (!result.containsKey(s)) {
                                IntListHard il = new IntListHard();
                                il.addNewPosition(lineNumber, wordNumber);
                                result.put(s, il);
                            } else {
                                IntListHard il = result.get(s);
                                il.addNewPosition(lineNumber, wordNumber);
                                result.put(s, il);
                            }
                        }
            ...
            List<Map.Entry<String, IntListHard>> entries = new ArrayList<>(result.entrySet());
            Comparator<Map.Entry<String, IntListHard>> sizeComparator = Comparator.comparingInt(entry -> entry.getValue().size());
            Collections.sort(entries, sizeComparator);
            for (Map.Entry<String, IntListHard> p : entries) {
